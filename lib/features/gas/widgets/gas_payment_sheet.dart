...
class _GasPaymentSheetState extends ConsumerState<GasPaymentSheet> {
  ...
  Future<void> _payWithWallet(double balance) async {
    ...
    try {
      ...
      final success    = await controller.deductForGasOrder(
        amount:      widget.total,
        description: 'Gas order — ${widget.cylinderSize.displayName} × ${widget.quantity}',
      );
      if (!mounted) return;
      if (success) {
  ...
  Future<void> _topUpAndPay(double balance) async {
    ...
    try {
      ...
      if (success) {
        ...
        if (!mounted) return;
        // Re-fetch balance directly
        final remote      = ref.read(walletRemoteDataSourceProvider);
        final freshWallet = await remote.getWalletByUserId(
            FirebaseAuth.instance.currentUser?.uid ?? '');
        final newBalance  = freshWallet?.balance ?? 0.0;
        if (newBalance >= widget.total) {
          await _payWithWallet(newBalance);
        } else {
          _showError('Balance insufficient after top-up. Please try again.');
        }
      }
    } catch (e) {
      if (mounted) _showError(e.toString().replaceFirst('Exception: ', ''));
                  ...
                  children: [
                    ...
                    GlassCard(
                      ...
                      child: Column(
                        children: [
                          _SummaryRow(
                            label:  '${widget.cylinderSize.displayName} × ${widget.quantity}',
                            amount: widget.gasPrice * widget.quantity,
                          ),
                          if (_brandPremium > 0) ...[
                            const SizedBox(height: 10),
                            _SummaryRow(
                              label:  'Brand premium (${widget.brand!.displayName})',
                              amount: _brandPremium,
                            ),
                          ],
                          const SizedBox(height: 10),
                          _SummaryRow(
                              label: 'Delivery fee', amount: widget.deliveryFee),
  ...
  Widget _buildBalanceCard(double balance, bool hasFunds) {
    final color = hasFunds ? Colors.green : Colors.orange;
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildPayButton(double balance) => SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: _isProcessing
              ...
              : Text('Pay ₵${widget.total.toStringAsFixed(2)}',
        ...
        child: ElevatedButton.icon(
          ...
          label: _isProcessing
              ? const SizedBox(
                  ...
                  child: CircularProgressIndicator(
                      strokeWidth: 2, color: Colors.white))
              : Text('Top Up & Pay ₵${widget.total.toStringAsFixed(2)}',
                  style: const TextStyle(
